...
const PublicationLogos = ({ subset, className = "" }: PublicationLogosProps) => {
  ...
  return (
    <div className={`flex flex-wrap justify-center items-center gap-4 md:gap-6 ${className}`}>
      {items.map((pub) =>
        pub.logo ? (
          <div key={pub.name} className={`flex items-center justify-center opacity-70 hover:opacity-100 transition-opacity ${pub.name === "The Sun" ? "h-8 md:h-10" : pub.name === "Bottoms Up! with Alan Carr" ? "h-20 md:h-24" : pub.name === "Daily Mail" ? "h-20 md:h-24" : pub.name === "Amazon Prime" ? "h-16 md:h-20" : pub.name === "BA High Life" ? "h-14 md:h-16" : "h-10 md:h-12"}`} title={pub.name}>
            <img
              src={pub.logo}
              alt={pub.name}
              className={`h-full w-auto object-contain ${pub.name === "The Sun" ? "max-w-[100px]" : pub.name === "Bottoms Up! with Alan Carr" ? "max-w-[200px]" : pub.name === "Daily Mail" ? "max-w-[280px]" : pub.name === "Amazon Prime" ? "max-w-[200px]" : pub.name === "BA High Life" ? "max-w-[180px]" : "max-w-[140px]"}`}
            />
          </div>
        ) : (
          <span
